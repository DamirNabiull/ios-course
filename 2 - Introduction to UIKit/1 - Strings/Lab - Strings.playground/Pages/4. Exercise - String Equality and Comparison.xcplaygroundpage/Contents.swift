 ...
 */
let nameInCapsLowercased = nameInCaps.lowercased()
let nameLowercased = name.lowercased()

if  nameInCapsLowercased == nameLowercased {
    print("\(nameInCapsLowercased) and \(nameLowercased) are the same.")
} else {
    print("\( nameInCapsLowercased) and \(nameLowercased) are not the same.")
}
...
func check(document: String, contains text: String) -> Bool {
    return document.lowercased().contains(text.lowercased())
}

if check(document: textToSearchThrough, contains: textToSearchFor) {
    print("I found it!")
}
